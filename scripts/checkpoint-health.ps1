...
if ($yamlContent -match "last_update:\s*\x22([^\x22]+)\x22") {
        try {
            $ageHours = ($now - [DateTime]::Parse($matches[1])).TotalHours
        } catch {
            # Unparseable date; flag as suspicious
            Write-Warning "checkpoint-health: unparseable last_update date, flagging as suspicious"
            $ageHours = 9999
        }
    }
...
$isGrowing = ($now - $lastTs).TotalMinutes -lt 30
    ...
    # Log growth calculation failed; flag as not growing
    Write-Warning "checkpoint-health: growth calculation failed - $($_.Exception.Message)"
    $isGrowing = $false
}
    }
    if ($isGrowing) {
