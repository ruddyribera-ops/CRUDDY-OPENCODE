...
## Output
- **Snapshot dir:** `<SCRATCH_DIR>/BEFORE_{timestamp}_{operation}\`
- **Log file:** `<SCRATCH_DIR>/BEFORE_LOG.txt` (appends each run)

## Refusal Rules
Tool **refuses** to backup if source is under:
- `<INSTALL_DIR>/` (agent's own files)
- `<SCRATCH_DIR>/` (backup location itself)

## Restore Procedure
To restore from a snapshot:
```powershell
# PowerShell
Copy-Item -Path "<SCRATCH_DIR>/BEFORE_20260618-143052_cleanup\*" -Destination "C:\target\" -Recurse -Force
