...
```
<INSTALL_DIR>/opencode.json
```

> Note: `<INSTALL_DIR>/scripts\hooks\hook-config.json` is NOT read by OpenCode. It was a prior attempt that wrote to the wrong path. The real config is `opencode.json` at the config root.
...
|-------|---------|-------------|---------|
| `sessionStart.memoryRetrieval` | SessionStart | `<INSTALL_DIR>/factory\hooks\session_start.py` | Injects relevant memories at session start |

## Adding a New Hook

1. Edit `<INSTALL_DIR>/Windows\.config\opencode\opencode.json`
...
"hooks": {
    "sessionStart": {
        "memoryRetrieval": {
            "enabled": true,
            "script": "<INSTALL_DIR>/factory\\hooks\\session_start.py",
...
python <INSTALL_DIR>/factory\hooks\session_start.py 2>$null
```
