---
name: Windows shell gotchas (machine-wide) — pwsh (PowerShell 7) FIRST, git bash second
description: Ruddy runs commands in pwsh 7.6.2. Every command the agent SHOWS for manual run must be PowerShell-compatible. Only internal Bash-tool calls can use POSIX.
type: feedback
---

# Rule of Thumb

**Default shell: pwsh (PowerShell 7.6.2).** pwsh handles `&&`, quote escaping, and UTF-8 correctly. Use powershell (5.1) only when pwsh is unavailable.

| Context | Shell | Syntax |
|---------|-------|--------|
| Agent runs it via the Bash tool | Git Bash (MINGW64) | POSIX works |
| Agent SHOWS user a command to copy-paste | **pwsh 7** | PowerShell syntax required |
...
| `cmd1 && cmd2` | `cmd1 && cmd2` (pwsh 7+) |
| `cmd1 \|\| cmd2` | `cmd1 && cmd2` (pwsh 7) or `cmd1; if (-not $?) { cmd2 }` |
...
1. **`&&` in user-facing commands** — safe in pwsh 7+. Default to `&&`. Use `; if ($?)` only if you know the user is on PS 5.1.
...
- **Self-check rule:** if you just typed `export`, `$VAR` (no `$env:`), or `<<EOF` in a command meant for the user — STOP. Translate first. `&&` is safe in pwsh 7.

## Why

Ruddy upgraded from PowerShell 5.1 to pwsh 7.6.2. Commands like `export DATABASE_URL=...` and `cmd1 && cmd2` paste into PowerShell and fail silently or loudly. The fix lives here so the coordinator and every specialist can find it.
